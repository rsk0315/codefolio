...
public:
  ...
  std::vector<std::pair<ForwardIt2, ForwardIt2>> search_all(
    ...
    for (size_type i = M_pat.size(), j = 0; i < pat.size();) {
      ...
      size_type k = 1;
      while (i+k < pat.size() && k < M_pat.size() && k+z[k] < j) z[i+k] = M_z[k], ++k;
      i += k;
      j -= k;
    }
    ...
    //   while (it != last && j < M_pat.size() && M_pred(M_pat[j], *it)) ++it, ++j;
    //   z[i] = j;
    //   if (j == 0) {
    //     ++i, ++it;
    //     continue;
    ...
    //   // i += k-1;
    //   while (it != last && i < M_pat.size() && k+M_z[k] < j) z[++i] = M_z[k], ++k, ++it;
    //   j -= k;
    // }
