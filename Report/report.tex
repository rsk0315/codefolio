\documentclass[a4paper]{jsarticle}

% symbols
\usepackage[T1]{fontenc}
\usepackage{textcomp}

% color
\usepackage[dvipdfmx]{color}

% font
\usepackage{lmodern}
\newcommand{\textlt}[1]{\texttt{\fontseries{l}\selectfont #1}}
\newcommand{\textmt}[1]{\texttt{\fontseries{m}\selectfont #1}}
\newcommand{\textbt}[1]{\texttt{\fontseries{b}\selectfont #1}}

% title-formatting
\usepackage{titlesec}
\titleformat*{\section}{\Large\bfseries\rmfamily}
\titleformat*{\subsection}{\large\bfseries\rmfamily}
\titleformat*{\subsubsection}{\normalsize\bfseries\rmfamily}

% toc-formatting
\usepackage{tocloft}
\renewcommand\cftsecafterpnum{\vskip0pt}
\renewcommand\cftsubsecafterpnum{\vskip0pt}
\renewcommand\cftsubsubsecafterpnum{\vskip0pt}

\renewcommand{\cftsecnumwidth}{4.0em}
\renewcommand{\cftsubsecnumwidth}{4.0em}
\renewcommand{\cftsubsubsecnumwidth}{4.0em}

\renewcommand{\cftsecfont}{\bfseries\rmfamily}
\renewcommand{\cftsubsecfont}{\mcfamily}
\renewcommand{\cftsubsubsecfont}{\mcfamily}

\renewcommand{\cftsecpagefont}{\bfseries\rmfamily}
\renewcommand{\cftsubsecpagefont}{\rmfamily}
\renewcommand{\cftsubsubsecpagefont}{\rmfamily}

% spacing
\setlength{\parskip}{.5\baselineskip}
\makeatletter
\titlespacing{\section}{0pt}%
             {.5\Cvs \@plus.5\Cdp \@minus.2\Cdp}{0pt \@plus.3\Cdp}
\titlespacing{\subsection}{0pt}%
             {.5\Cvs \@plus.5\Cdp \@minus.2\Cdp}{0pt \@plus.3\Cdp}
\titlespacing{\subsubsection}{0pt}%
             {.5\Cvs \@plus.5\Cdp \@minus.2\Cdp}{0pt \@plus.3\Cdp}
\newcount\ch
\ch=0\loop
  \ifnum\ch<256
  \xspcode \ch=3
\advance\ch by1
\repeat

% source-code
\usepackage{listings}
\lstset{
  basicstyle=\ttfamily\footnotesize,
  frame=single,
  flexiblecolumns=true,
  upquote=true,
  keepspaces=true,
}

\usepackage{enumerate}

% math
\usepackage{euler}
\usepackage{amsmath}
\usepackage{mathtools}
\usepackage[dvipsnames]{xcolor}

\thinmuskip=5mu
\SetMathAlphabet{\mathrm}{normal}{T1}{ccr}{m}{n}
\DeclareSymbolFont{concreteop}{T1}{ccr}{m}{n}
\makeatletter
\renewcommand{\operator@font}{\mathgroup\symconcreteop}
\makeatother

% linking
\usepackage{tablefootnote}
\usepackage{footnotehyper}

\usepackage[
  %% This option breaks S-<left> and S-<right> in evince,
  %% but commenting it out disables hyperlinking, ugh
  %% dvipdfmx,
  linktocpage=true,
  plainpages=false,
]{hyperref}
\usepackage[dvipdfmx]{pxjahyper}
\hypersetup{
  dvipdfmx,
  colorlinks=true,
  allcolors=blue,
  pdftitle={Report},
}

% framing
\usepackage{framed}

\title{
  {\Huge 一日一テーマやるやつ}\\
  {\Large 一日で終わらなければ次の日も}
}
\author{
  えび
}
\begin{document}
\maketitle

\pagenumbering{roman}
\newpage
\tableofcontents

\newpage
\pagenumbering{arabic}
\setcounter{section}{0}
\renewcommand{\thesection}{\arabic{section}}

\section{データ構造}
\input{veb-tree}
\input{rb-tree}
\input{cb-tree}
\input{sp-array}
\input{sp-uf}
\input{quick-find}
\input{weighted-qf}

\newpage
\section{グラフ理論}
\input{dc-online}

\newpage
\section{構文解析}
\input{cyk}
\input{tsurai-parser}

\include{todo}

\end{document}
