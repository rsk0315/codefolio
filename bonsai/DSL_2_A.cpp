...
template <class Tp>
struct update {
  Tp operator ()(const Tp&, const Tp& y) const { return y; }
};

template <class Tp>
struct find {
  Tp identity = std::numeric_limits<Tp>::max();
  Tp operator ()(const Tp& x, const Tp& y) const { return std::min(x, y); }
};
...
int main() {
  ...
  segment_tree<int, find<int>, update<int>> st(n);
