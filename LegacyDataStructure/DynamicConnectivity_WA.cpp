...
public:
  ...
  std::set<size_t> component(size_t u) const {
    auto it=nodes.find({u, u});
    if (it == nodes.end()) return {};
    ...
    const RbNode *u_first=it->second->first();
    for (const RbNode *p=u_first; p; p=p->successor()) {
      res.insert(p->value.first);
      if (p->value.first != p->value.second)
        res.insert(p->value.second);
    }
...
public:
  ...
  void cut(size_t u, size_t v) {
    ...
    size_t ll=level[std::minmax(u, v)];
    if (!fs[0].contains(u, v)) {
    ...
    for (size_t i=ll+1; i--;) {
      ...
      for (size_t uu: tu) {
        for (auto it=t_edges[i][uu].begin(); it!=t_edges[i][uu].end();) {
          ...
          ++it;
          if (x < y)
            increase_level(x, y);  // Note: this invalidates the iterator (it)
        }
      }
  ...
  void increase_level(size_t u, size_t v) {
